...
import { generateWithGroq } from "@/lib/chatbotAi";
...
const emptyChatbot: Chatbot = {
  ...
  description: "",
  status: "inactive",
  personality: "Amable, clara y profesional",
  objective: "",
  welcome_message: "",
  system_prompt: "",
  business_hours: "",
  handoff_instructions:
    "Transferir a una persona cuando el usuario lo solicite o la consulta exceda la información disponible.",
  required_fields: [],
  faqs: [],
};
...
function Onboarding({ userName, done }: { userName: string; done: () => void }) {
  ...
  return (
    <main className="min-h-screen grid place-items-center p-6">
      <form onSubmit={submit} className="glass-card rounded-2xl p-8 max-w-lg w-full">
        <Building2 className="text-primary mb-5" />
        <p className="text-primary text-xs tracking-widest uppercase">Un último paso</p>
        <h1 className="text-3xl mt-2">Hola {userName || ""}, configura tu empresa</h1>
        <p className="text-muted-foreground mt-3 mb-7">
          Esta empresa será el espacio privado donde vivirán tus agentes y llamadas.
        </p>
        ...
        <button
          disabled={busy}
          className="gradient-bg w-full rounded-xl py-3 flex justify-center gap-2"
        >
          {busy && <Loader2 className="animate-spin" size={18} />}Crear espacio de trabajo
        </button>
      </form>
    </main>
  );
...
function Overview({
  ...
  const today = calls.filter(
    (c) => new Date(c.started_at).toDateString() === new Date().toDateString(),
  );
  const minutes = Math.round(today.reduce((s, c) => s + c.duration_seconds, 0) / 60);
  const resolved = today.length
    ? Math.round(
        (today.filter((c) => c.result && c.result !== "Sin respuesta").length / today.length) * 100,
      )
    : 0;
  return (
    <>
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        <Stat
          ...
          detail="Actividad del día"
          icon={PhoneCall}
        />
        <Stat
          label="Minutos procesados"
          value={String(minutes)}
          detail="Tiempo de conversación"
          icon={Clock3}
        />
        <Stat
          label="Tasa de resolución"
          value={`${resolved}%`}
          detail="Llamadas con resultado"
          icon={Activity}
        />
        <Stat
          label="Agentes activos"
          value={String(agents.filter((a) => a.status === "active").length)}
          detail={`${agents.length} configurados`}
          icon={Mic2}
        />
      </div>
      <div className="admin-panel">
        <div className="panel-head">
          <div>
            ...
            <p>Actividad más reciente de tus agentes</p>
          </div>
          <button onClick={() => onSection("calls")}>Ver todas →</button>
        </div>
        <CallTable calls={calls.slice(0, 6)} />
      </div>
    </>
  );
}
function Stat({
  ...
  return (
    <article className="admin-panel p-5 relative">
      <Icon className="absolute right-5 top-5 text-blue-500 bg-blue-50 p-2 rounded-xl" size={38} />
...
function Chatbots({
  ...
  return (
    <div className="admin-panel">
      <div className="panel-head">
        <div>
          <h2>Constructor de chatbots</h2>
          <p>Describe el negocio, genera la configuración y pruébala antes de publicar</p>
        </div>
        <button className="admin-primary" onClick={onNew}>
          <Plus size={15} />
          Nuevo chatbot
      ...
      {bots.length === 0 ? (
        <Empty
          icon={Bot}
          title="Aún no tienes chatbots"
          action="Crea el primero escribiendo lo que necesitas"
        />
      ) : (
        <div className="divide-y">
          {bots.map((b) => (
            <article key={b.id} className="agent-row">
              <span className="agent-icon">
                <Bot />
              </span>
              <div>
                <span className={b.status === "active" ? "status-pill" : "status-pill inactive"}>
                  {b.status === "active" ? "Activo" : "Borrador"}
                </span>
                <h3>{b.name}</h3>
                <p>{b.objective || b.description || "Sin objetivo configurado"}</p>
              </div>
              <Data label="PERSONALIDAD" value={b.personality} />
              <Data label="CANAL" value="Simulador web" />
              <button
...
function buildDraft(description: string): Chatbot {
  const clean = description.trim();
  const words = clean
    .replace(/[^a-záéíóúñ0-9 ]/gi, "")
    .split(/\s+/)
    .filter(Boolean);
  const name = words.slice(0, 4).join(" ") || "Nuevo chatbot";
  return {
    ...emptyChatbot,
    name: `Asistente ${name}`.slice(0, 120),
    description: clean,
    objective: `Atender consultas relacionadas con: ${clean}`,
    welcome_message: "¡Hola! Soy el asistente virtual. ¿En qué puedo ayudarte hoy?",
    system_prompt: `Eres un asistente virtual amable, claro y profesional. Tu objetivo es atender consultas sobre el siguiente negocio o necesidad: ${clean}. Responde únicamente con información confirmada. Si no sabes una respuesta, indícalo con honestidad y ofrece transferir la conversación a una persona. No inventes precios, políticas ni disponibilidad.`,
    required_fields: ["Nombre", "Teléfono o correo"],
    faqs: [],
  };
}
...
function ChatbotEditor({
  ...
  const { send: streamReply, streaming: sending } = useChatStream();
  const [message, setMessage] = useState("");
  const [chat, setChat] = useState<{ role: "user" | "assistant"; content: string }[]>([]);
  const set = <Key extends keyof Chatbot>(key: Key, value: Chatbot[Key]) =>
    setForm((f) => ({ ...f, [key]: value }));
  const generate = async () => {
    if (!form.description.trim()) return toast.error("Primero describe el chatbot que necesitas");
    setGenerating(true);
    try {
      const generated = await generateWithGroq(form.description);
      setForm((current) => ({
        ...current,
        ...generated,
        id: current.id,
      ...
      toast.success("Configuración generada con IA. Revísala antes de guardar.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "No se pudo generar la configuración");
    } finally {
      setGenerating(false);
    }
  };
  const submit = async () => {
    ...
    const { error } = chatbot.id
      ? await supabase.from("chatbots").update(payload).eq("id", chatbot.id)
      : await supabase.from("chatbots").insert(payload);
    setBusy(false);
    if (error) return toast.error(error.message);
    toast.success(chatbot.id ? "Chatbot actualizado" : "Chatbot creado");
    saved();
  };
  const send = async () => {
    ...
    try {
      await streamReply(form, next, (token) => {
        setChat((current) => {
      ...
      const faq = form.faqs.find((f) => lower.includes(f.question.toLowerCase()));
      const answer =
        faq?.answer ||
        `Gracias por tu mensaje. ${form.objective || "Estoy aquí para ayudarte."} ${form.handoff_instructions ? "Si lo necesitas, puedo ayudarte a contactar a una persona." : ""}`;
      setChat([...next, { role: "assistant", content: answer }]);
      toast.warning("Groq no respondió; se utilizó la respuesta local.");
    }
  };
  return (
    <div className="modal-layer" onMouseDown={close}>
      <div className="editor chatbot-editor" onMouseDown={(e) => e.stopPropagation()}>
        <button className="editor-close" onClick={close}>
          <X />
        </button>
        <p className="text-[10px] tracking-widest text-blue-600">
          {chatbot.id ? "EDITAR CHATBOT" : "CONSTRUCTOR DE CHATBOT"}
        </p>
        <h2 className="text-2xl font-semibold mt-1">
          {chatbot.id ? `Editar ${chatbot.name}` : "¿Qué chatbot necesitas?"}
        </h2>
        <p className="text-sm text-slate-500 mb-5">
          Describe el negocio en lenguaje natural y genera una base editable.
        </p>
        <label className="block text-xs font-semibold text-slate-600">
          ...
          <textarea
            ...
            placeholder="Ej. Quiero un chatbot para una clínica dental que responda horarios, tratamientos y solicite citas."
          />
        </label>
        <button
          type="button"
        ...
        <div className="editor-grid mt-5">
          <label>
            Nombre
          ...
          <label>
            ...
            <select
              value={form.status}
              onChange={(e) => set("status", e.target.value as Chatbot["status"])}
            >
          ...
          <label className="wide">
            Transferencia a persona
            <textarea
              rows={2}
              value={form.handoff_instructions}
              onChange={(e) => set("handoff_instructions", e.target.value)}
            />
          </label>
        </div>
        <section className="mt-6 border rounded-2xl p-4 bg-slate-50">
          ...
          <p className="text-xs text-slate-500 mb-3">
            Prueba con Groq el tono y el comportamiento antes de publicar.
          </p>
          <div className="bg-white border rounded-xl p-3 min-h-28 max-h-48 overflow-auto space-y-2">
            {chat.length === 0 ? (
              <p className="text-sm text-slate-400">
                {form.welcome_message || "Genera la configuración para comenzar la prueba."}
              </p>
            ) : (
              chat.map((m, i) => (
                <p
                  key={i}
                  className={`text-sm rounded-xl p-2 ${m.role === "user" ? "bg-blue-600 text-white ml-10" : "bg-slate-100 mr-10"}`}
                >
                  {m.content}
                </p>
              ))
          ...
          <div className="flex gap-2 mt-2">
            ...
            />
            <button className="admin-primary" type="button" onClick={send} disabled={sending}>
        ...
        <footer>
          ...
          <button
            ...
            onClick={submit}
          >
            {busy && <Loader2 className="animate-spin" size={16} />}Guardar chatbot
          </button>
        </footer>
      </div>
    </div>
    ...
    <div className="admin-panel">
      <div className="panel-head">
        <div>
          ...
          <p>Resultados y trazabilidad de conversaciones</p>
        </div>
      </div>
      {calls.length ? (
        ...
        <Empty
          ...
          action="Aparecerán aquí cuando el VPS envíe actividad"
        />
      )}
    </div>
  );
    ...
    <div className="overflow-auto">
      <table className="admin-table">
        ...
        <tbody>
          {calls.map((c) => (
            <tr key={c.id}>
              <td>{c.voice_agents?.name || "Agente"}</td>
              <td>{c.caller_phone || "Privado"}</td>
              <td>
                <span className="status-pill">{c.result || c.status}</span>
              </td>
              <td>
...
function Profile({
  ...
  return (
    <div className="admin-panel p-6 max-w-2xl">
      <h2 className="text-lg font-semibold">Perfil y empresa</h2>
      <p className="text-sm text-slate-500 mb-6">Información asociada a tu espacio de trabajo.</p>
      <div className="grid sm:grid-cols-2 gap-5">
        <DataCard label="Nombre" value={profile?.full_name || "Sin nombre"} />
        <DataCard label="Correo" value={email} />
        <DataCard label="Empresa" value={org.name} />
        <DataCard label="Industria" value={org.industry || "Sin especificar"} />
      </div>
    </div>
  );
}
const DataCard = ({ label, value }: { label: string; value: string }) => (
  <div className="border rounded-xl p-4">
    <small className="text-slate-400 block">{label}</small>
