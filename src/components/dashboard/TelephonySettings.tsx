...
export default function TelephonySettings() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "No se pudieron actualizar los números.",
      );
    } finally {
      setBusy(null);
    }
  ...
  if (loading) {
    return (
      <div className="admin-panel p-10 grid place-items-center">
      ...
      <div className="grid xl:grid-cols-[1.1fr_.9fr] gap-6">
        <form onSubmit={connect} className="admin-panel p-6">
          ...
          <p className="text-sm text-slate-500 mt-2 mb-6">
            Usa una API Key revocable. El Secret se envía una sola vez al backend y se cifra en
            Vault.
          </p>
    ...
    <div className="space-y-6">
      <section className="admin-panel p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            className="border rounded-xl px-4 py-2 text-sm text-red-600 flex items-center gap-2"
            onClick={disconnect}
        ...
        {state.numbers.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-sm text-slate-400">
              QubeSight no comprará números durante esta fase.
            </p>
          </div>
        ) : (
              ...
              <article key={number.provider_number_sid} className="agent-row">
                ...
                <div className="hidden lg:block">
                  ...
                  <strong className="text-xs">
                    {number.capabilities?.voice ? "Compatible" : "No indicada"}
                  </strong>
                </div>
                <div className="hidden lg:block">
                  <small className="text-[9px] tracking-wider text-slate-400 block">
                    WEBHOOK ACTUAL
                  </small>
                  <strong className="text-xs">
                    {number.current_voice_url ? "Ya configurado" : "Sin configurar"}
                  </strong>
                </div>
                <button
                ...
                >
                  {busy === number.provider_number_sid && (
